...
for metric_name in metrics_names:
    ...
    for column_name in columns_list:
        ...
        df_corr = pd.DataFrame(columns=['dataset','data', 'r', 'p_value', 'MSE'])
        df_corr_log = pd.DataFrame(columns=['dataset','data', 'r', 'p_value', 'MSE'])
        for dataset_name,dataset_title, session, timestamp_1, timestamp_2 in zip(datasets_name, dataset_titles, sessions, timestamps_1, timestamps_2):
            ...
            for sex in ['F', 'M']:
                ...
                df_corr_log = pd.concat([df_corr_log, pd.DataFrame({'dataset':[dataset_title],
                                                            ...
                                                            'MSE': [mse]})], axis=0)
        # Save the results
        df_corr.to_csv(os.path.join(path_to_save_tables, f'{column_name}_correlation_with_class_distinctiveness.csv'), index=False)
        df_corr_log.to_csv(os.path.join(path_to_save_tables, f'{column_name}_correlation_with_log_class_distinctiveness.csv'), index=False)
