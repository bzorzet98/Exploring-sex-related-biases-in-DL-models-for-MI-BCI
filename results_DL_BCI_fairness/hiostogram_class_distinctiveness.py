...
for dataset_name,row_name, session in zip(datasets_name, row_names, sessions):
    ...
    if os.path.exists(os.path.join(DIRECTORY_TO_SAVE_ROOT, f'{experiment_name}', 'distinctiveness_coefficent_per_subject')):
        ...
        # PLot a histogram
        ax[0,it_col] = sns.histplot(data=metadata, x='class_distinctiveness',ax=ax[0,it_col])
        ax[0,it_col].set_title(row_name)
        ax[0,it_col].set_xlabel('Class Distinctiveness')
        ax[0,it_col].set_ylabel('Frequency')


        sns.histplot(data=metadata, x='log_class_distinctiveness',ax=ax[1,it_col])  
        ax[1,it_col].set_title(row_name)
        ax[1,it_col].set_xlabel('Log Class Distinctiveness')
        ax[1,it_col].set_ylabel('Frequency')
        it_col += 1
...
fig.tight_layout()
plt.savefig(os.path.join(path_to_save, 'class_distinctiveness_histogram.png'))
plt.close()
